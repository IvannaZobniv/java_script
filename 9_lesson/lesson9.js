...
let simpsons = [
    ...
    },
    {
        ...
        info: 'Ма́ргарет Эвелин «Мэ́гги» Си́мпсон (англ. Margaret Evelyn “Maggie” Simpson) — персонаж мультсериала ' +
            ...
            'через три года семья Симпсонов получила собственный сериал на телеканале Fox, дебют произошёл 17 декабря' +
            '1989 года.',
...
for (const  family  of simpsons){
    const {name, surname, age, info, photo} = family;
    let div = document.createElement('div');
    div.classList.add('member');

    let familyDiv = document.createElement('div');
    familyDiv.innerText = `${name}--${surname}--${age}--${info}`;

    let familyImg = document.createElement('img');
    familyImg.src = photo;

    div.append(familyDiv,familyImg);
...
document.body.append(inputOne,inputTwo,inputThree,button3);
